...
class Sqlite {
  static Future<sql.Database> _initDatabase() async {
    ...
    return sql.openDatabase(path.join(sqlPath, 'localDatabase.db'), onCreate: (db, version) {
      return db.execute('''CREATE TABLE tasks (id INTEGER PRIMARY KEY AUTOINCREMENT,
          uid TEXT NOT NULL, 
        body TEXT NOT NULL,
  ...
  Future<int> delete(int id) async {
    final db = await Sqlite._initDatabase();
    return await db.delete('tasks', where: 'id = ? AND uid = ?', whereArgs: [id, Constants.uid]);
  }
